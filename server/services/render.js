...
exports.logedin_home = async(req, res) => {
    ...
    if(userdata.data.length != 0 &&  userdata.data[0].password == req.body.password){
        ...
        res.render('home',{admin:false, login : true, user : userdata.data , product : allproductdata.data, myProduct : productdata.data});
    } else {
...
exports.product_view = async(req, res) => {
    ...
    try {
        let productdata = await axios.get(`http://localhost/api/products?id=${id}`);
        res.render('ProductView', { product : productdata.data });
    } catch (error) {
        res.send(error);
    }
}
...
exports.auction = async(req, res) => {
    ...
    try {
        ...
        res.render('liveAuction', { user : userdata.data, product : productdata.data, bid : biddata.data });
    } catch (error) {
        res.send(error);
    }
}